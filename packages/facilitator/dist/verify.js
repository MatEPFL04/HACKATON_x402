...
// ============================================================
export async function verifyBoc(paymentPayload, paymentDetails, client) {
    try {
        ...
        const searchResult = await deepSearchCells(cell, expectedAddress, BigInt(paymentDetails.amount), paymentDetails.asset, claimedFrom, paymentPayload.queryId, client);
        ...
        return {
            valid: false,
            reason: `BOC parsing error: ${err.message}`,
        };
    }
}
async function deepSearchCells(cell, targetAddress, minAmount, asset, senderAddress, queryId, client, visited = new Set()) {
    ...
    for (let i = 0; i < cell.refs.length; i++) {
        const childResult = await deepSearchCells(cell.refs[i], targetAddress, minAmount, asset, senderAddress, queryId, client, visited);
        if (childResult.foundAddress)
            foundAddress = true;
    }
    return { foundAddress };
}
