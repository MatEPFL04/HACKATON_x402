...
export async function settleBoc(paymentPayload, paymentDetails, options) {
    ...
    try {
        ...
        while (Date.now() - startTime < timeoutMs) {
            ...
            try {
                ...
                console.log(`[settle] Poll #${pollCount} error: ${e.message}`);
            }
        }
...
function matchTransaction(tx, fromAddress, expectedAmount, queryId) {
    const inMsg = tx.inMessage;
    if (!inMsg)
        return false;
    if (inMsg.info.type !== "internal")
