import { decodePaymentRequired, encodePaymentPayload, decodeSettlementResponse, generateQueryId, atomicToJetton, nanoToTon, HEADER_PAYMENT_REQUIRED, HEADER_PAYMENT_SIGNATURE, HEADER_PAYMENT_RESPONSE, } from "@ton-x402/core";
import { internal, external, beginCell, storeMessage, Address, SendMode, } from "@ton/core";
// ============================================================
// Verbose logger
// ============================================================
const c = {
...
function headerBox(name, raw, decoded) {
...
function arrow(direction, color, label) {
    console.log(`\n  ${c.bold}${color}${direction}${c.reset} ${label}`);
}
// ============================================================
// Core: create signed BOC for a TON transfer
// ============================================================
async function createSignedBoc(wallet, keypair, seqno, to, amount, queryId, asset = "TON", client) {
    let transferMessage;
    if (asset === "TON") {
        ...
        const jettonTransferBody = beginCell()
            ...
            .storeCoins(1000000n) // forward_ton_amount (0.001 TON for notification)
 ...
 */
export async function x402Fetch(url, config, init) {
    ...
    const tonOption = paymentRequired.accepts.find((a) => a.scheme === "ton-v1");
    if (!tonOption) {
        throw new Error(`No TON payment option available. Accepted schemes: ${paymentRequired.accepts.map((a) => a.scheme).join(", ")}`);
    }
    // ── Step 2: Determine seqno ──────────────────────────────────────
    let seqno = config.seqno;
    if (seqno === undefined) {
        if ("getSeqno" in wallet && typeof wallet.getSeqno === "function") {
            seqno = await wallet.getSeqno();
        }
        else {
            throw new Error("seqno not provided and wallet has no getSeqno() method. " +
    ...
    const boc = await createSignedBoc(wallet, keypair, seqno, config.payTo ?? tonOption.payTo, config.amount ?? tonOption.amount, queryId, tonOption.asset, config.client);
